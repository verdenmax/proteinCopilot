...
> **版本**：1.0
> **创建日期**：2026-03-27
> **状态**：In Progress — M1.1 ✅ M1.2 ✅ M1.3 ✅ M1.4 ✅ M1.5 ✅ M1.6 ✅ M1.7 ✅（838 tests, 0 warnings）— MVP 完成 + Post-MVP 功能（异步搜索优化、搜索历史持久化、谱图注释可视化）+ DIA 数据支持（前体提取 + 搜索集成 + 端到端工作流 + 单谱图母离子提取）+ Biology Audit 完成 + BUG-1~16 已修复（碎片离子固定修饰 + DIA precursor m/z + Sage RT 单位 + PPM 零值保护 + DIA 搜索安全 + 未打分 PSM + RT 标签 + Decoy 检测统一化 + unwrap 安全化 + HTML 注入防护 + Plotly 版本统一 + 魔法数字常量化）+ 28 MCP Tools + FW-1 ✅ ProteinNTerm 修饰过滤 + FW-2 ✅ 可变修饰枚举 + FW-3 ✅ ppm 碎片离子容差 + FW-4 ✅ 多电荷碎片离子 + FW-6 ✅ 原生 FDR 计算（fdr crate + decoy 生成 + q-value 单调化）+ XIC 可视化（xic crate + extract_xic tool + SILAC 重标轻重离子 + Plotly.js HTML）+ 索引谱图读取（IndexedMzMLReader / IndexedMgfReader + scan offset 随机访问）+ 外部搜索结果导入（result-import crate + import_search_results tool + DIA-NN parquet / custom JSON / RT 扫描匹配）+ 统一标注+XIC 视图（客户端 SILAC 引擎 + 逐离子 L/H 开关 + DDA 自动跳过 XIC）+ M2.0 ✅ Sage 搜索引擎集成（sage-core v0.15.0 库集成 + rayon 并行打分 + LDA rescoring + spectrum/peptide/protein FDR）+ M2.2 ✅ 独立 FDR 计算 + M2.3 ✅ 蛋白推断（Parsimony + Razor）+ M2.4 ✅ FASTA 数据库管理 + RT 二分查找优化（ScanIndex + PCIX v2 + find_by_rt O(log N) + collect_ms2_info 零 I/O）+ ✅ 陷阱库分析 v1+v2（L0-L4 分级 + Levenshtein edit distance + k-mer 预筛 + SubstitutionType 注释 + HTML 报告 + CLI + 3 MCP tools）+ ✅ 陷阱库分析 v3（碎片离子溯源 + 嵌合谱检测 + UniMod 修饰 + RT-based scan lookup + 镜像图可视化）+ ✅ 陷阱库分析 v4（多目标碎片溯源 + CoElutionIndex + 轻重标 SILAC 搜索 + DIA 双扫描镜像 + MirrorData 结构 + Per-PSM HTML 报告增强 + 噪声过滤 + 边界修复）+ ✅ 跨模块流程审计（DIA precursor 全链路修复 + SILAC 轻重标 MS2 分离验证）+ ✅ 代码审计修复（unwrap 安全化 + HTML 注入防护 + Plotly.js 2.35.2 统一 + 魔法数字常量化）+ ✅ 溯源报告增强（RT + q-value + 修饰展示 + Heavy 计数 + 嵌合检测改进）
...
> **完成情况总览**（2026-04-08 审计）：
>
> | 模块 | P0 需求 | P1 需求 | P2 需求 | 完成率 |
> |------|---------|---------|---------|--------|
> | FR-1 谱图读取 | 5/5 ✅ | 1/1 ✅ | — | **100%** |
> | FR-2 参数推荐 | 7/7 ✅ | — | — | **100%** |
> | FR-3 搜索引擎 | 4/5 ⚠️ | 2/2 ✅ | — | **86%**（pFind 桩） |
> | FR-4 报告生成 | 4/4 ✅ | 1/1 ✅ | 0/1 ❌ | **83%**（缺 compare） |
> | FR-5 Agent/Skill | 5/5 ✅ | — | — | **100%** |
> | FR-6 FASTA 管理 | 1/1 ✅ | 1/1 ✅ | 1/1 ✅ | **100%** ✅ fasta-db crate + 3 MCP tools |
> | FR-7 共享结构 | 7/7 ✅ | — | — | **100%** |

### FR-1：谱图读取模块（spectrum-io）— ✅ 全部完成
...
### FR-2：参数推荐模块（param-recommend）— ✅ 全部完成
...
### FR-3：搜索引擎调度模块（search-engine）— ✅ SimpleEngine 完成 + Sage 生产级搜索

| ID | 需求 | 优先级 | 状态 |
|---|---|---|---|
| FR-3.1 | 实现 `SearchEngineAdapter` trait，定义统一的搜索接口 | P0 | ✅ 含 search, health_check, search_with_spectra, cancel |
| FR-3.2 | 实现 pFind adapter：将 SearchParams 转换为 pFind .cfg 配置格式 | P0 | ❌ **桩代码**，search() 返回 "not yet implemented" |
| FR-3.3 | 通过 SSH 连接远程服务器执行 pFind（`tokio::process::Command` + SSH） | P0 | ❌ **桩代码**，SshConfig 已定义但未实现 |
| FR-3.4 | 解析 pFind 输出结果文件，转换为标准化 `SearchResult` | P0 | ❌ **未实现** |
| FR-3.5 | 实现搜索进度跟踪（轮询 pFind 日志/进度文件） | P1 | ✅ SimpleEngine 有进度回调；pFind 日志轮询未实现 |
| FR-3.6 | 实现 health_check：检查远程 pFind 是否可达、版本信息 | P0 | ✅ pFind 返回 Unavailable，SimpleEngine 返回 Healthy |
| FR-3.7 | 实现 `EngineRegistry`：管理多个搜索引擎 adapter | P1 | ✅ register/get/list_available/health_check_all |
...
### FR-4：报告生成模块（report）— ⚠️ 核心完成，缺 compare
...
| FR-4.6 | 比较两次搜索结果的差异（鉴定数变化、overlap） | P2 | ❌ **未实现**（推迟） |

**MCP Tools**：
- `generate_summary`：输入 SearchResult → 输出 SearchResultSummary
- `export_results`：输入 SearchResult + format + output_path → 输出 ExportResult

### FR-5：Agent 与 Skill 定义 — ✅ 全部完成

| ID | 需求 | 优先级 | 状态 |
|---|---|---|---|
| FR-5.1 | 定义 `proteomics-search.agent.md`：蛋白质搜索助手，编排完整搜索流程 | P0 | ✅ 引用全部 14 个 MCP tools |
...
| FR-5.4 | Agent 必须在执行搜索前请求用户确认参数 | P0 | ✅ Agent 指令中明确要求（代码层无强制） |
| FR-5.5 | Agent 必须先调用 MCP Tool 获取数据再做推荐/解释，不得"凭空"推断 | P0 | ✅ Agent 指令中明确要求 |

> **额外完成**：除 PRD 要求的 3 个 Skill 外，还实现了 `dia-precursor-extraction.prompt.md` 和 `spectrum-annotation.prompt.md`（共 5 个蛋白组学 Skill）+ 3 个开发辅助 Skill

### FR-6：FASTA 数据库管理 — ✅ 全部完成

| ID | 需求 | 优先级 | 状态 |
|---|---|---|---|
| FR-6.1 | 支持用户指定本地 FASTA 路径 | P0 | ✅ fasta.rs 解析本地文件 |
| FR-6.2 | 内置常用数据库列表（UniProt Human、Mouse、E.coli 等） | P1 | ✅ fasta-db crate, 6 内置数据库, list_databases 工具 |
| FR-6.3 | 支持从 UniProt HTTPS 自动下载并缓存 | P2 | ✅ download_database + SHA256 校验 + 本地缓存 |

### FR-7：共享数据结构（core crate）— ✅ 全部完成
...
| # | 排除项 | 未来阶段 | 实际状态（2026-04-08） |
|---|---|---|---|
| NG-1 | 独立的 FDR 控制模块（`mcp-fdr`）——MVP 依赖 pFind 自带的 FDR 计算 | Phase 2 | ⚠️ report/summary.rs 中有 1% FDR 过滤，但非独立模块 |
| NG-2 | 数据质控模块（`mcp-qc`）——谱图质量评估和异常检测 | Phase 2 | ❌ 未实现 |
| NG-3 | 蛋白推断模块（`mcp-protein-inference`）——Parsimony 算法 | Phase 2 | ❌ 未实现 |
| NG-4 | MSFragger / Comet 等其他搜索引擎 adapter | Phase 2 | ❌ 未实现 |
| NG-5 | 搜索失败的自动诊断 Agent——诊断参数不合理、数据质量差等原因 | Phase 2 | ❌ 未实现 |
| NG-6 | 多轮分析对话——在一次会话中迭代调参、重搜、比较 | Phase 3 | ⚠️ 异步搜索 + 搜索历史提供基础支撑，但无会话状态管理 |
| NG-7 | 开放搜索 / 未知修饰发现 | Phase 3 | ⚠️ open_search 预设已有（500Da tolerance），但无修饰发现逻辑 |
| NG-8 | 定量分析支持（Label-free / TMT / iTRAQ） | Phase 3 | ❌ 未实现 |
| NG-9 | 谱图可视化（SVG/HTML）和交互式报告 | Phase 3 | ✅ **已提前实现** — visualize.rs + annotate_spectrum MCP tool |
| NG-10 | Web UI / GUI——MVP 仅通过 MCP Client（Copilot CLI 等）交互 | 未规划 | ❌ 未实现 |
...
> **状态**：✅ 已完成（174 tests, 0 clippy warnings）
...
- **实现补充**：`SearchParams` 额外 derive `PartialEq`（支持一致性校验）。`SearchParamsError` 枚举（8 变体：EmptyDatabasePath, InvalidPrecursorTolerance, InvalidFragmentTolerance, TooManyMissedCleavages, InvalidModificationMassDelta, TooManyVariableMods, ZeroPeptideLength, InvalidPeptideLengthRange）。新增字段 `acquisition_mode: Option<AcquisitionMode>`、`max_variable_modifications: u32`（默认 3）、`min_peptide_length: u32`（默认 7）、`max_peptide_length: u32`（默认 50）。
...
> **状态**：✅ 已完成（54 tests, 0 clippy warnings，已通过 165MB 真实 DIA mzML 验证）
>
...
> **状态**：✅ 已完成（38 tests = 34 unit + 4 integration, 0 clippy warnings）
>
...
- **实现补充**：ParamRecommendError 枚举（2 变体：EmptySummary, InvalidSummary）+ `From<ParamRecommendError> for CoreError`

#### Task 1.3.2：定义 UserHints 和 SearchPreset ✅

- **Sub-task 1.3.2.1**：定义 `UserHints` 结构体（experiment_type: Option\<String\>, instrument_type: Option\<String\>, enzyme: Option\<Enzyme\>, custom_notes: Option\<String\>），derive JsonSchema
- **Sub-task 1.3.2.2**：定义 `SearchPreset` 结构体（name, description, params: SearchParams, applicable_scenarios: Vec\<String\>）+ `with_database()` 方法
- **Sub-task 1.3.2.3**：实现 **5 个**内置预设：
  ...
  - `silac`：SILAC 标记搜索（同位素标记模式）
  - `open_search`：开放搜索（500Da precursor tolerance）
...
> **状态**：✅ 已完成（72 tests, 0 warnings）— 采用 SimpleSearchEngine + Sage adapter（sage-core v0.15.0 生产级搜索）+ pFind 预留结构 + 碎片离子固定修饰已修复
>
...
> **状态**：✅ 已完成（12 tests, 0 warnings）
>
> 搜索结果的统计聚合与导出，含谱图注释 HTML 可视化。纯计算，不依赖 MCP。
...
> **状态**：✅ 已完成（14 MCP tools 注册，Agent + 5 Skill 编写完成，2 tests）
>
...
> **状态**：⚠️ 部分完成 — 文档已编写，端到端测试通过代码测试（e2e_integration.rs 8 tests），但未完成用户手动测试场景
>
> 验证完整流程，编写文档。
> 关联 SM：SM-1, SM-7

#### Task 1.7.1：端到端测试场景 ⚠️ 部分

- **Sub-task 1.7.1.1**：准备测试数据：小型 mgf（100 张谱图）+ 小型 FASTA（100 蛋白） ✅ （fixture 文件存在）
- **Sub-task 1.7.1.2**：场景 A（自动化流程）：用户说"搜索这个数据" → Agent 执行完整流程 → 验证输出报告 ⚠️ （代码测试通过，未做用户手动测试）
- **Sub-task 1.7.1.3**：场景 B（指定参数）：用户直接给参数 → 跳过推荐 → 验证搜索执行 ⚠️ （同上）
- **Sub-task 1.7.1.4**：场景 C（探查数据）：用户只想看谱图摘要 → 验证 read_spectra 返回正确 ✅ （集成测试覆盖）
- **Sub-task 1.7.1.5**：记录所有问题和优化点到 GitHub Issues ❌ 未执行

#### Task 1.7.2：文档 ✅

- **Sub-task 1.7.2.1**：更新 `README.md`：项目简介、快速开始（安装 → 配置 SSH → 运行）、架构图 ✅
- **Sub-task 1.7.2.2**：编写 `docs/mcp-tools.md`：每个 MCP Tool 的名称、输入 schema、输出 schema、使用场景、示例 ✅（14 tools 全部文档化）
...
### Milestone 2.0：Sage 搜索引擎集成 ✅

> **状态**：✅ 已完成（562 tests, 0 warnings）— sage-core v0.15.0 库集成

- **SageAdapter**：实现 `SearchEngineAdapter` trait，支持 `search()` 和 `search_with_spectra()` 两种调用模式
- **类型转换层**（convert.rs）：Spectrum → RawSpectrum、MassTolerance → sage Tolerance、Modification → ModificationSpecificity
- **配置构建器**（config.rs）：SearchParams → sage Parameters/Builder，支持 7 种酶、固定/可变修饰、decoy 策略
- **搜索流程**：FASTA 解析 → IndexedDatabase 构建 → rayon 并行打分 → LDA rescoring（带 NaN 保护）→ spectrum/peptide/protein q-value → 标准化 SearchResult
- **MCP 集成**：`run_search` 支持 `engine: "Sage"` 参数（大小写不敏感），DIA 和文件两条路径均已对接
- **进度报告**：500ms 轮询式进度回调，与 run_cache 集成
- **代码审查**：修复 2 critical + 4 important + 3 suggestion 级问题
- **边界修复**：蛋白 FDR 过滤、peptide_count 语义修正、DIA 结果 metadata 填充、错误路径 engine info 修正
...
> **状态**：✅ 已完成 — fdr crate 实现 target-decoy FDR + q-value 单调化

- 实现 target-decoy FDR 计算
- PSM / 肽段 / 蛋白质多级 FDR
- q-value 单调化
- Sage adapter 内置 LDA rescoring + sage-core FDR pipeline

### Milestone 2.3：`mcp-protein-inference` — 蛋白推断 ✅

> **状态**：✅ 已完成 — protein-inference crate + infer_proteins MCP Tool

- Parsimony 蛋白推断算法（最小蛋白集覆盖所有肽段）
- Razor 肽段分配（共享肽段分配给最大证据蛋白组）
- 序列覆盖率计算（FASTA 支持）
- MCP Tool：`infer_proteins`

### Milestone 2.4：多搜索引擎支持 — ⚠️ Sage 完成，MSFragger/Comet 待实现

> Sage adapter 已完成（M2.0）。MSFragger、Comet adapter 及多引擎结果合并待后续实现。

- ~~MSFragger adapter~~（待实现）
- ~~Comet adapter~~（待实现）
- ~~多引擎结果合并与一致性分析~~（待实现）
...
| 2026-04-22 | 陷阱库 HTML 报告修复 7 项（BUG-10：Heavy 离子归属修正 + 列数修正 + JSON 转义增强 + Plotly 2.35.2 + RT 列新增 + JS 索引同步 + viewport meta）| entrapment 报告质量审计 |
| 2026-04-24 | 代码审计修复（BUG-11~16）：tools.rs unwrap 安全化 + unimod.rs unwrap 安全化 + diann.rs Regex LazyLock + xic_visualize.rs 肽段 HTML 转义 + entrapment report.rs JSON 转义增强 + 3 个魔法数字常量化（DIA_ISOLATION_WINDOW_THRESHOLD_DA / RT_AUTO_LOOKUP_TOLERANCE_MIN / FDR_1PCT_THRESHOLD）；Plotly.js 全项目统一 2.35.2；838 tests | 五模块并行代码审计 + 修复 |
| 2026-04-24 | 溯源报告增强：MultiTargetProvenance 新增 trap_retention_time_min + trap_q_value 字段；Per-PSM header 展示 RT 和 q-value；Summary 表新增 Heavy 计数列（H:TrapOnly/H:Shared/H:TargetOnly）；Candidate 表替换冗余 Spectrum File 为 Modifications 列；嵌合检测改用轻+重合并 shared fraction | 溯源报告数据完整性增强 |
...
| BUG-10 | **entrapment HTML Heavy 离子归属错误** | Heavy mirror 从 `target_peptide` 改为 `trap_peptide` + `shift_ions_heavy()`；Heavy `matched_count` 使用 `heavy.matched_peaks` 而非 light 的值；mirror_plot.rs 同步修复 | ✅ 已修复 |
| BUG-11 | **tools.rs L1765 unwrap 崩溃** | `match` 替代 `get_result().unwrap()`，失败返回结构化 ErrorData | ✅ 已修复 |
| BUG-12 | **unimod.rs L129 unwrap 崩溃** | `if let` 替代 `unwrap()`，解析失败跳过而非 panic | ✅ 已修复 |
| BUG-13 | **diann.rs Regex::new().unwrap()** | 改为 `LazyLock` 静态编译，零运行时 panic 风险 | ✅ 已修复 |
| BUG-14 | **xic_visualize.rs 肽段注入风险** | 肽段序列在写入 HTML `<title>` 前进行 HTML 转义（`<>&"` → 实体） | ✅ 已修复 |
| BUG-15 | **entrapment report.rs JSON 转义不完整** | 从仅 `</` 转义升级为全 `<>` → `\u003c\u003e` 转义，与 xic 模块对齐 | ✅ 已修复 |
| BUG-16 | **tools.rs 魔法数字硬编码** | 提取为命名常量：`DIA_ISOLATION_WINDOW_THRESHOLD_DA=5.0`、`RT_AUTO_LOOKUP_TOLERANCE_MIN=0.5`、`FDR_1PCT_THRESHOLD=0.01` | ✅ 已修复 |
...
- **功能**: 集成 sage-core v0.15.0 作为 Rust 库依赖，提供生产级蛋白质组学搜索
- **架构**: SageAdapter 实现 SearchEngineAdapter trait，三层模块结构（convert / config / mod）
- **搜索流程**: FASTA → IndexedDatabase → rayon 并行打分 → LDA rescoring → spectrum/peptide/protein FDR
- **类型转换**: Spectrum ↔ RawSpectrum, MassTolerance ↔ Tolerance, Modification ↔ ModificationSpecificity
- **配置**: SearchParams → sage Parameters，支持 7 种酶、固定/可变修饰、decoy 策略映射
- **MCP 集成**: `run_search` 支持 `engine: "Sage"` 参数（大小写不敏感），DIA 和文件路径均已对接
- **进度**: 500ms 轮询进度回调，与 run_cache 集成
- **结果扩展**: hyperscore, matched_peaks, discriminant_score 等 Sage 特有指标通过 PSM.extra 传递
- **代码审查修复**: 蛋白 FDR 计数修正、LDA NaN 保护、peptide_count 语义修正、错误路径 engine info 修正
- **测试**: 562 tests 通过（+52 from Sage adapter, 含 12 convert + 7 config + 22 mod + 8 integration + 3 integration_test）
