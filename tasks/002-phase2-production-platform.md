...
> **版本**：1.0
> **创建日期**：2026-04-16
> **状态**：In Progress — M2.0 ✅ Sage 集成完成, M2.4 ✅ 蛋白推断完成, M2.12 ✅ 搜索失败诊断完成
...
MVP 阶段验证了 **Rust MCP Server + LLM Agent** 的全流程可行性：25 个 MCP 工具、13 个 crate、670+ 测试、DDA/DIA 双模式支持。但 MVP 使用的 SimpleSearchEngine（b/y 离子匹配）仅适合功能验证，无法用于真实科研。
...
#### M2.0：Sage 搜索引擎集成 ✅ 已完成

**目标**：集成 Sage (Rust 原生搜索引擎)，一个集成获得 DDA+DIA+LFQ+TMT+FDR 五大能力。

**状态**：✅ P0 全部完成（2026-04-17）。sage-core v0.15.0 库集成，rayon 并行打分 + LDA rescoring。
...
| ID | 任务 | 状态 | 说明 |
|----|------|------|------|
| M2.0.1 | **添加 sage-core crate 依赖** | ✅ | sage-core 0.15.0, sage-cloudpath 0.3.8 |
| M2.0.2 | **SearchParams → SageConfig 转换器** | ✅ | config.rs: enzyme/mods/tolerance/DIA 全映射 |
| M2.0.2a-d | → enzyme/mods/tolerance/DIA 映射 | ✅ | 7 酶 + 固定/可变修饰 + ppm/Da + wide_window |
| M2.0.3 | **SageAdapter 实现 SearchEngineAdapter trait** | ✅ | search()/search_with_spectra() + diagnostics |
| M2.0.3a-d | → PSM/Protein/decoy/错误 转换 | ✅ | convert.rs: Feature→Psm, 蛋白提取, is_decoy 映射 |
| M2.0.4 | **注册 SageAdapter 到 EngineRegistry** | ✅ | create_adapter("sage") → SageAdapter |
| M2.0.5 | **health_check 实现** | ✅ | 返回 sage-core 版本 |
| M2.0.6 | **engine_info 返回 Sage 元数据** | ✅ | name="Sage", version="0.15.0" |
| M2.0.7 | **MCP 工具适配：run_search 支持 Sage** | ✅ | engine="Sage" 参数 |
| M2.0.12 | **单元测试：config 转换** | ✅ | 32 个测试覆盖所有参数组合 |
| M2.0.13 | **集成测试：端到端搜索** | ✅ | mzML + FASTA → Sage 搜索验证 |

**未实现（降级到后续）**：
- M2.0.8 param-recommend 自动推荐 Sage（P1）
- M2.0.9 LFQ 定量包装（P1，依赖 M2.6 定量模块）
- M2.0.10 TMT 定量包装（P1，依赖 M2.6）
- M2.0.11 DIA chimeric 模式测试（P1）
...
**实现方式**：与原计划不同，诊断功能集成到 `core` crate 的 `diagnostics` 模块中（而非 `param-recommend`），并通过搜索引擎 adapter trait 进行阶段级指标采集。

**已完成任务**：

| ID | 任务 | 状态 | 说明 |
|----|------|------|------|
| M2.12.1 | **失败模式分类规则** | ✅ | `core/src/diagnostics.rs`: ErrorCategory(InputData/Parameters/Database/Engine) |
| M2.12.1a | → 低 ID 率诊断 | ✅ | rate < 10% → warning, < 5% → DatabaseMismatch |
| M2.12.1b | → 高 FDR / 零 PSM 诊断 | ✅ | psms_at_1pct_fdr == 0 → error; < 50 → warning |
| M2.12.1c | → 容差异常诊断 | ✅ | < 5 ppm + low rate → NarrowTolerance; > 50 ppm → WideTolerance |
| M2.12.1d | → 数据库不匹配 | ✅ | rate < 5% + no error → DatabaseMismatch heuristic |
| M2.12.2 | **诊断决策树** | ✅ | `finalize()` 方法: 7 条规则 + 幂等保护 + 建议排序 |
| M2.12.3 | **自动参数调整建议** | ✅ | DiagnosticSuggestion: priority + action + reason + param_changes JSON |
| M2.12.4 | **一键重搜** | ⏳ | 降级到后续（需要 prepare_search 集成） |
| M2.12.5 | **Agent 更新** | ✅ | proteomics-search.agent.md: 搜索诊断工作流 section |
| M2.12.6 | **Prompt 模板** | ✅ | failure-diagnosis.prompt.md: LLM 诊断解释指南 |
| — | **SearchEngineAdapter trait 更新** | ✅ | search()/search_with_spectra() 接受 &mut SearchDiagnostics |
| — | **SimpleSearch 阶段指标** | ✅ | 4 阶段: fasta_parsing, digestion, matching, fdr_calculation |
| — | **Sage 阶段指标** | ✅ | 3 阶段: file_reading, fasta_parsing, matching |
| — | **pFind stub 诊断** | ✅ | 返回 ErrorCategory::Engine |
| — | **MCP tool: diagnose_search** | ✅ | 第 25 个 tool, 返回完整诊断报告 |
| — | **SearchProgress 增强** | ✅ | error_category + has_diagnostics 字段 |
| — | **RunState 诊断存储** | ✅ | 成功路径 finalize() + 失败路径 error_category |
| — | **边界加固** | ✅ | fail_stage 幂等 + double-finalize 保护 + 零 PSM 处理 |
...
| ~~`diagnose_search`~~ | ~~diagnostics~~ | ✅ 已实现（M2.12） |
...
| 阶段 | Milestone | 依赖 | 复杂度 | 状态 |
|------|-----------|------|--------|------|
| **Phase 2a** | **M2.0** Sage 集成 | 无 | 高 | ✅ 完成 |
| **Phase 2a** | **M2.4** 蛋白推断 | 无 (可并行) | 高 | ✅ 完成 |
| **Phase 2a** | **M2.7** QC 模块 | 无 (可并行) | 中 | ⏳ 部分完成（工作流优化） |
| **Phase 2b** | **M2.1** pFind 对接 | 需要 pFind 二进制 | 高 | 未开始 |
| **Phase 2b** | **M2.2** DIA-NN 对接 | 需要 DIA-NN 二进制 | 高 | 未开始 |
| **Phase 2b** | **M2.3** 批处理 | M2.0 或 M2.1 | 高 | 未开始 |
| **Phase 2c** | **M2.5** Rescoring | M2.0 或 M2.1 | 中 | 未开始 |
| **Phase 2c** | **M2.6** 定量 | M2.4 | 高 | 未开始 |
| **Phase 2c** | **M2.8** PTM 定位 | M2.0 或 M2.1 | 中 | 未开始 |
| **Phase 2d** | **M2.9** 结果比较 | 任一引擎 | 低 | 未开始 |
| **Phase 2d** | **M2.10** 多引擎扩展 | M2.0+M2.1 | 中 | 未开始 |
| **Phase 2d** | **M2.11** 标准导出 | M2.4 | 中 | 未开始 |
| **Phase 2d** | **M2.12** 失败诊断 | 任一引擎 | 低 | ✅ 完成 |
| **Phase 2d** | **M2.13** AI 增强 | 前置完成 | 低 | 未开始 |
...
1. ✅ **M2.0 Sage 集成** — Rust crate 依赖，无需额外安装（2026-04-17 完成）
2. ✅ **M2.4 蛋白推断** — 纯算法，仅依赖 PSM 结果（2026-04-16 完成）
3. ✅ **M2.7 QC 模块** — 谱图质量评估不依赖搜索引擎（工作流优化 2026-04-17 完成）
4. ✅ **M2.9 结果比较** — 已有 SearchResult 结构
5. ✅ **M2.11 mzTab 导出** — 格式转换
6. ✅ **M2.12 搜索失败诊断** — 结构化诊断 + MCP tool + Agent prompt（2026-04-18 完成）
...
| 2026-04-16 | M2.4 蛋白推断完成 | parsimony + razor + 三级 FDR + 覆盖率 |
| 2026-04-17 | M2.0 Sage 集成完成 | sage-core 0.15.0 集成，P0 全部完成 |
| 2026-04-17 | 工作流优化完成 | prepare_search + DIA 缓存溢出 + Agent 更新 |
| 2026-04-18 | M2.12 搜索失败诊断完成 | 结构化诊断 + 7 条异常规则 + diagnose_search tool |
| 2026-04-18 | 平台指标更新 | 13 crates, 25 tools, 12 prompts, 670+ tests |
