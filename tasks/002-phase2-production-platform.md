...
**目标**：扩展搜索引擎 adapter，支持 DDA 和 DIA 主流引擎。

| ID | 需求 | 优先级 | 说明 |
|----|------|--------|------|
| M2.9.1 | MSFragger adapter | P1 | DDA 搜索引擎，SearchEngineAdapter 实现 |
| M2.9.2 | Sage adapter | P1 | Rust 原生搜索引擎，架构最匹配，开源 |
| M2.9.3 | DIA-NN adapter | P1 | DIA 事实标准，已有 Parquet 结果导入基础 |
| M2.9.4 | Comet adapter | P2 | DDA 搜索引擎，SearchEngineAdapter 实现 |
| M2.9.5 | 多引擎结果合并 | P2 | iProphet 风格的多引擎评分整合 |

> **DIA 引擎策略**：
> - DIA-NN 通过 `--lib` 参数接受谱图库，`--fasta` 接受 FASTA 文件，CLI 接口清晰
> - MVP 已实现 `import_search_results` 的 `diann_parquet` 格式，M2.9.3 在此基础上增加进程编排
> - AlphaDIA 的 Python 生态可通过子进程 + TOML 配置文件驱动，但优先级低于 DIA-NN
> - Sage (Rust) 可通过库依赖直接集成，无需子进程，架构最优
