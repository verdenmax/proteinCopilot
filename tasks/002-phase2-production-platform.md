...
| ID | 任务 | 优先级 | 涉及文件 | 验收标准 | 依赖 |
|----|------|--------|----------|----------|------|
| M2.4.1 | **ProteinGroup 数据结构** | P0 | `core/src/protein_group.rs` (新) | leader, members, peptides, unique_peptides, is_decoy | 无 |
| M2.4.1a | → ProteinGroup struct | P0 | 同上 | Serialize + Deserialize + JsonSchema | — |
| M2.4.1b | → InferenceResult struct | P0 | 同上 | `{ groups: Vec<ProteinGroup>, razor_map: HashMap<String, String> }` | — |
| M2.4.2 | **肽段-蛋白映射构建** | P0 | `protein-inference/src/mapper.rs` (新) | PSM list → `HashMap<peptide_seq, Vec<protein_accession>>` | 无 |
| M2.4.2a | → I/L 等价处理 | P0 | 同上 | Isoleucine (I) / Leucine (L) 视为等价 | — |
| M2.4.2b | → decoy 蛋白分离 | P0 | 同上 | target / decoy 蛋白分开建映射 | — |
| M2.4.3 | **Parsimony 算法实现** | P0 | `protein-inference/src/parsimony.rs` (新) | 贪心最小蛋白集覆盖所有 peptide evidence | M2.4.2 |
| M2.4.3a | → 贪心集合覆盖 | P0 | 同上 | 每步选择覆盖最多未解释肽段的蛋白 | — |
| M2.4.3b | → 子集蛋白合并 | P0 | 同上 | A 的所有肽段 ⊆ B → A 合并到 B 的 protein group | — |
| M2.4.3c | → 不可区分蛋白分组 | P0 | 同上 | 肽段集完全相同的蛋白 → 同一 protein group | — |
| M2.4.4 | **Razor 肽段分配** | P0 | `protein-inference/src/razor.rs` (新) | 共享肽段分配给 unique 肽段最多的 protein group | M2.4.3 |
| M2.4.4a | → Razor 算法 | P0 | 同上 | 按 unique peptide count 降序分配 shared peptides | — |
| M2.4.4b | → 歧义标记 | P0 | 同上 | 无法唯一分配时标记为 `ambiguous` | — |
| M2.4.5 | **肽段级 FDR** | P0 | `fdr/src/peptide_fdr.rs` (新) | 按 best PSM score 对 unique peptide 排序 → TDC q-value | 无 |
| M2.4.5a | → unique peptide 提取 | P0 | 同上 | 每个 sequence 取 best score PSM 代表 | — |
| M2.4.5b | → peptide-level TDC | P0 | 同上 | target-decoy competition → peptide q-value | — |
| M2.4.6 | **蛋白级 FDR (picked-protein)** | P0 | `fdr/src/protein_fdr.rs` (新) | picked-protein approach，target/decoy 配对竞争 | M2.4.3 |
| M2.4.6a | → target-decoy protein pairing | P0 | 同上 | 每个 target protein 与其 reverse decoy 配对 | — |
| M2.4.6b | → picked protein competition | P0 | 同上 | 配对中选 score 更高的 → 计算 protein-level FDR | — |
| M2.4.6c | → protein group score | P0 | 同上 | group score = best peptide score 或 sum top-3 | — |
| M2.4.7 | **序列覆盖率计算** | P1 | `protein-inference/src/coverage.rs` (新) | 从 FASTA + identified peptides → 氨基酸级覆盖率 % | M2.4.3 |
| M2.4.7a | → FASTA 序列加载 | P1 | 同上 | 从 FASTA 读取蛋白序列 | — |
| M2.4.7b | → 肽段位置定位 | P1 | 同上 | 每个 peptide 在蛋白序列中的 start/end position | — |
| M2.4.7c | → 覆盖率百分比 | P1 | 同上 | covered_residues / total_residues × 100% | — |
| M2.4.8 | **MCP 工具：infer_proteins** | P0 | `mcp-server/src/tools.rs` | 新 MCP tool，输入 run_id → 输出 InferenceResult | M2.4.3 |
| M2.4.9 | **MCP 工具：get_protein_groups** | P1 | `mcp-server/src/tools.rs` | 查看蛋白分组详情 | M2.4.8 |
| M2.4.10 | **单元测试：Parsimony** | P0 | `protein-inference/src/parsimony.rs` | ≥10 个手工构造用例（含 shared/unique/subset/identical） | M2.4.3 |
| M2.4.11 | **单元测试：Razor** | P0 | `protein-inference/src/razor.rs` | ≥5 个测试用例 | M2.4.4 |
| M2.4.12 | **单元测试：protein FDR** | P0 | `fdr/src/protein_fdr.rs` | picked-protein 正确性验证 | M2.4.6 |
| M2.4.13 | **推断过程可视化 (HTML)** | P2 | `report/src/inference_visualize.rs` (新) | 蛋白-肽段映射关系图 | M2.4.3 |
