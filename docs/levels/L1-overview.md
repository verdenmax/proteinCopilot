...
```text
质谱文件 (mgf/mzML) + FASTA
  ① spectrum-io        读取解析 → SpectrumSummary（索引随机访问）
  ② param-recommend    推荐参数 → AiDecision<SearchParams>
  ③ search-engine      酶切→匹配→打分 → SearchResult（SimpleSearch + Sage）
  ④ fdr                target-decoy q-value（PSM/肽/蛋白 1%）
  ⑤ protein-inference  parsimony + razor + 蛋白级 FDR + 覆盖率
  ⑥ report             统计摘要 + TSV/JSON 导出
  ⑦ xic                碎片离子 XIC 提取 + Plotly 可视化（含 SILAC）
  ⑧ dia-extraction     DIA 母离子提取（宽窗同位素分析）
  ⑨ result-import      外部结果导入（DIA-NN / pFind / custom JSON）
...
- **接口**：约 25 个 MCP Tool，覆盖读谱、推参、搜索（异步 run_id）、状态、注释、XIC、导入、推断、数据库、entrapment。
