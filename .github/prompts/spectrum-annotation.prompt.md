...
| XIC 探测 | `for_each_spectrum()` **全文件流式扫描** | **瓶颈：100-150s/7.5GB** |
| XIC+raw 提取 | **第二遍全文件流式扫描** | **瓶颈：同上** |

### 关键认知

- **单 scan 查找已经 O(1)**：`IndexedMzMLReader` 使用磁盘缓存（`.mzML.idx` sidecar），首次打开时 SIMD byte-scan 构建索引并持久化，后续打开毫秒级加载
- **XIC 是主要瓶颈**：每次标注都全文件扫描提取 XIC 色谱数据。7.5GB 文件 ≈ 100-150 秒/次
- **N 个肽段 = N 次全文件扫描**：批量标注 12 个肽段 ≈ 25 分钟
...
2. **直接传 scan_number**：如果用户已知 scan number，直接使用，跳过任何查找
3. **RT 查找（后备）**：`scan_number=0 + retention_time_min` 需要索引二分查找，比直接传 scan number 多一步但仍很快

### 批量标注建议

- 先用 `export_results` 一次获取所有目标肽段的 scan number、charge、modifications
- 顺序调用 `annotate_spectrum`（服务端有 reader LRU 缓存，同一文件复用索引）
- 大文件（>5GB）注意 MCP client 超时设置（`.mcp.json` 中 `timeout` 建议 ≥ 600s）
- 当前每次标注仍需全文件扫描 XIC，这是已知待优化项
